...
class Trace():
	...
	def stats(self, alpha=0.05, start=0, batches=100,
		...
		if chain is None:
			...
			stat['%s%s HPD interval' % (int(100 * (1 - alpha)), '%')] = self.calc_hpd_multchain(alpha)
		else:
			if chain < 0:
				chain = xrange(self.db.chains)[chain]
			filtered_rdd = self.db.rdd.filter(lambda x: x[0]==chain).map(lambda x: x[1][tname]).cache()
			stat['mean'] = filtered_rdd.map(lambda x: x.mean(0)).first()
			stat['standard deviation'] = filtered_rdd.map(lambda x: x.std(0)).first()
			stat['mc error'] = self.batchsd(chain=chain, batches=batches, rdd=filtered_rdd)
			...
			stat['%s%s HPD interval' % (int(100 * (1 - alpha)), '%')] = filtered_rdd.map(hpd_map_helper).first()
		return stat
	...
	def multiplechain_batchsd(self, batches=5):
		...
		def reduce_helper(x, y):
			weighted_mean = np.divide(np.multiply(x[1], x[2]) + np.multiply(y[1], y[2]), x[2]+y[2], dtype=float)
	...
	def batchsd(self, chain=-1, batches=5, rdd=None):
		...
		if rdd is None:
			tname = self.name
			rdd = self.db.rdd.filter(lambda x: x[0]==chain).map(lambda x: x[1][tname]).cache()
		def batchsd_helper(trace):
			if batches > len(trace):
				return 'Could not generate mc error'
			if len(np.shape(trace)) > 1:

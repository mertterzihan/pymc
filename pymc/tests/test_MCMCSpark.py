...
class test_MCMCSpark_withHDFS(TestCase):
	...
	def test_sample(self):
		...
		for chain in xrange(10):
			assert_array_equal(self.M.trace('early_mean', chain=chain)[:].shape, (5,))
			assert_equal(self.M.trace('early_mean', chain=chain).length(), 5)
			assert_equal(self.M.trace('early_mean', chain=chain)[:].__class__, np.ndarray)
		...
		assert_array_equal(self.M.trace('early_mean', chain=None)[:].shape, (50,))
		assert_equal(self.M.trace('early_mean').length(chain=None), 50)
		assert_equal(self.M.trace('early_mean').gettrace(slicing=slice(1, 2)), db.trace('early_mean')[1])

...
class MCMCSpark():
    ...
    def sample(
        ...
        rdd = self.sc.parallelize(
            xrange(
                self.nJobs)).map(sample_on_spark).cache()
    ...
    def _check_database_backend(self, db):
        ...
        if not isinstance(db, str):
            raise ValueError(
                'MCMCSpark supports just strings as parameter for database backend for now')
        elif db not in Supported_Backends:
            raise ValueError(
                'MCMCSpark supports currently Spark (db=\'spark\') and HDFS (db=\'hdfs\') as database backends')
        self.save_to_hdfs = False
        if db == 'hdfs':
            self.save_to_hdfs = True
    ...
    def save_as_pickle_object(self, filename, batch_size=10, chain=None):
        ...
        if chain is None:
            ...
            self.db.rdd.filter(
                lambda x: x[0] == chain).saveAsPickleFile(
                filename,
                batch_size)
    ...
    def save_as_txt_file(self, path, chain=None):
        ...
        for v in self._variables_to_tally:
            ...
            temp_rdd.map(
                lambda x: (
                    x[0],
                    x[1][v])).map(save_mapper).saveAsTextFile(
                os.path.join(
                    path,
                    v))
        temp_rdd.map(
